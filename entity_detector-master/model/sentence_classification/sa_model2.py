...
class SentimentAnalysisModel2(Model):
    """ Simplest TF model """

    def __init__(self, config):
        super(SentimentAnalysisModel2, self).__init__(config)

    def build(self, n_tags=1):
        self.logger.info("Building " + SentimentAnalysisModel2.__name__)
    ...
    def set_lstm(self):
        ...
        with tf.variable_scope("bi-lstm1"):
            ...
            '''
            cell_fw_1 = tf.contrib.rnn.LSTMCell(self.config.hidden_size_lstm)
            cell_bw_1 = tf.contrib.rnn.LSTMCell(self.config.hidden_size_lstm)
            (output_fw_1, output_bw_1), (output_state_fw_1, output_state_bw_1) = tf.nn.bidirectional_dynamic_rnn(
                cell_fw_1,
                cell_bw_1,
                self.embeddings,
                sequence_length=self.sequence_lengths,
                dtype=tf.float32)
            #output_1 = tf.concat([output_state_fw_1[0], output_state_bw_1[0]], 1) ### ToDo DUDA con la indexación!
            output_1 = tf.concat([output_fw_1, output_bw_1], axis=2)
            output_1 = tf.nn.dropout(output_1, self.config.dropout) #Add dropout


        with tf.variable_scope("bi-lstm2"):
            cell_fw_2 = tf.contrib.rnn.LSTMCell(self.config.hidden_size_lstm2)
            cell_bw_2 = tf.contrib.rnn.LSTMCell(self.config.hidden_size_lstm2)
            (output_fw_2, output_bw_2), (output_state_fw_2, output_state_bw_2) = tf.nn.bidirectional_dynamic_rnn(
                cell_fw_2,
                cell_bw_2,
                output_1,
                sequence_length=self.sequence_lengths,
                dtype=tf.float32)
            output_2 = tf.concat([output_state_fw_2[0], output_state_bw_2[0]], 1)  ### ToDo DUDA con la indexación!
            output_2 = tf.nn.dropout(output_2, self.config.dropout)  # Add dropout


        ### Projection over our decission space: hidden state --> tag space
        with tf.variable_scope("proj"):
            W = tf.get_variable("W", dtype=tf.float32,
                                shape=[2 * self.config.hidden_size_lstm2, self.config.ntags])

            b = tf.get_variable("b", shape=[self.config.ntags],
                                dtype=tf.float32, initializer=tf.zeros_initializer())

            output = tf.reshape(output_2, [-1, 2 * self.config.hidden_size_lstm2])  ###[numSentences, 2*hiddenState]
            ### 'Linear layer'
    ...
    def get_predictions_tass(self, test,  processing_word=None, word2idx=None, char2idx=None):
        """ Predicts class for unseen instances

        :return: predicted classes
        """
        import xml.etree.ElementTree as ET
        tweets = ET.parse(test.filename)
        sentences = []
        ids = []
        ...
        ##### Here we have all tweets, with its tweetId and tag if provided #####
