...
class SentimentAnalysisModelN(Model):
    """ Simplest TF model """

    def __init__(self, config):
        super(SentimentAnalysisModelN, self).__init__(config)

    def build(self, n_tags=1):
        self.logger.info("Building " + SentimentAnalysisModelN.__name__)

        # Graph assembling
        self.set_placeholders()
        self.set_embeddings()
        self.set_lstm(self.config.layers, self.config.hidden_sizes)
        self.set_loss()
    ...
    def set_lstm(self, nlayers=2, hidden_sizes=[10,10]):
        """ Defines self.logits
            ...
            See: https://stackoverflow.com/questions/46189318/how-to-use-multilayered-bidirectional-lstm-in-tensorflow
        """

        output = self.embeddings
        for layer in range(nlayers):
            with tf.variable_scope('bi-lstm_{}'.format(layer), reuse=tf.AUTO_REUSE):
                ...
                cell_fw = tf.contrib.rnn.LSTMCell(hidden_sizes[layer],
                                                  initializer=tf.truncated_normal_initializer(-0.1, 0.1, seed=2))
                cell_fw = tf.contrib.rnn.DropoutWrapper(cell_fw, input_keep_prob=self.config.dropout)

                cell_bw = tf.contrib.rnn.LSTMCell(hidden_sizes[layer],
                                                  initializer=tf.truncated_normal_initializer(-0.1, 0.1, seed=2))
                cell_bw = tf.contrib.rnn.DropoutWrapper(cell_bw, input_keep_prob=self.config.dropout)

                output, states = tf.nn.bidirectional_dynamic_rnn(cell_fw,
                                                                  cell_bw,
                                                                  output,
                ...
                output = tf.concat([output_fw, output_fw], axis=2)
                #else:
                #   (state_fw, state_bw) = states
                #    output = tf.concat([state_fw[0], state_bw[0]], 1)
        output = output[:,-1,:]
