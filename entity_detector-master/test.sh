...
#curl  -X POST  "http://127.0.0.1:4999/ner" -d '{"words":"Cual es la dirección del restaurante El Baños", "plain":true}' -H "Content-Type: application/json; charset=utf-8"
#wget -qO-  "http://127.0.0.1:4999/ner?words=$i&other=True&plain=true" 
results=0
echo --- person ---
for i in "${person_array[@]}"; do
    echo -ne $i
    test=$(curl -s  -X POST  "http://127.0.0.1:4998/ner" -d '{"words":"'"$i"'", "plain":true}' -H "Content-Type: application/json; charset=utf-8" | jq   ' .["entities"][] | select(.entity=="person")| .value ' )
    echo -ne $test
    [ ${#test} -le 5 ] && echo -ne  " error " && results=2 || echo -ne  " Ok "
    echo
done
...
for i in "${location_array[@]}"; do 
    echo -ne  $i
    test=$(curl -s  -X POST  "http://127.0.0.1:4998/ner" -d '{"words":"'"$i"'", "plain":true}' -H "Content-Type: application/json; charset=utf-8"   | jq   ' .["entities"][] | select(.entity=="location")| .value ' )
    echo -ne $test
    [ ${#test} -le 5 ] && echo -ne " error " && results=2 || echo -ne " Ok "
    echo
done
...
echo --- organizacion --- 
for i in "${organizacion_array[@]}"; do
    echo -ne $i
    test=$(curl -s -X POST  "http://127.0.0.1:4998/ner" -d '{"words":"'"$i"'", "plain":true}' -H "Content-Type: application/json; charset=utf-8" | jq   ' .["entities"][] | select(.entity=="organization")| .value ' )
    echo -ne $test
    [ ${#test} -le 5 ] && echo -ne " error " && results=2 || echo -ne " Ok "
    echo
done
